		...
		<div class="page-wrapper">
			<div class="page-content">
				...
				<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3" style="margin-top: -20px">
					<div class="breadcrumb-title pe-3">Not Set Carton Size Product</div>
				...
				<h6 class="mb-0 text-uppercase">
					<form method="get" action="<?php echo PROJECTURL ?>/notsetpackingsizeproduct2">
						@csrf
					<div class="row">
						...
						<div class="col-sm-2  col-12">
							...
							<a href="<?php echo PROJECTURL ?>/notsetpackingsizeproduct" class="btn btn-primary" style="padding:2px !important;font-size:15px;height:30px">Clear</a>
						</div>
					</div>
				   </form>
				</h6>
					...
					<div class="card-body">
						<div class="">
							<table id="" class="table table-striped table-bordered" >
								...
								<tbody>
			<?php $oldcompany="";?>
			@foreach($productdata as $prddata)
			<?php 
...
if($oldcompany != $prddata->company_code)
{
$compcode=$prddata->company_code;
//get last sync
date_default_timezone_set("Asia/Karachi");
$currentdate=date('Y-m-d');
$compname="";
$fg_company="SELECT company_title FROM `ipl_company_master` where company_code=$prddata->company_code";
$run_company=mysqli_query($con,$fg_company);
if(mysqli_num_rows($run_company) !=0)
{
	$row_company=mysqli_fetch_array($run_company);
	$compname=$row_company['company_title'];

}

	echo "
<tr style='background-color:#679149'>
<th colspan='2'>Company : ".$prddata->company_code." - ".$compname."</th>
</tr>
	";
} 
...
?>						
<tr>
								<td style="font-size:12px;color:black">{{$prddata->company_code}} - {{$compname}} </td>
								<td style="font-size:12px;color:black">{{$prddata->product_code}} - {{$prddata->product_name}}</td>
								
									</tr>
<?php $oldcompany=$prddata->company_code;
 ?>
