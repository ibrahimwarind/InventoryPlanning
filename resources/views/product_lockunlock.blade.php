		...
		<div class="page-wrapper">
			<div class="page-content">
				...
				<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3" style="margin-top: -20px">
					<div class="breadcrumb-title pe-3">Product Lock/UnLock Management</div>
				...
				<h6 class="mb-0 text-uppercase">
					<form method="get" action="<?php echo PROJECTURL ?>/productlockunlock2">
						@csrf
					<div class="row">
						...
						<div class="col-sm-2  col-12">
							...
							<a href="<?php echo PROJECTURL ?>/productlockunlock" class="btn btn-primary" style="padding:2px !important;font-size:15px;height:30px">Clear</a>
						</div>
					</div>
				   </form>
				</h6>
						...
						<div class="">
							...
							<table id="example" class="table table-striped table-bordered" >
								<thead>
                                    ...
                                    <th style="text-align:center">Active/InActive for All DSF</th>
									<th style="text-align:center">Active/Inactive for Particular DSF</th>	
								...
								<tbody>
                                    ...
                                    <?php 
                                    $compnamename="";$price=0;
                                    $fg_comp="select * from sb_company_master where company_code='$user->company_code'";
                                    $run_comp=mysqli_query($con,$fg_comp);
                                    if(mysqli_num_rows($run_comp) !=0)
                                    {
                                        $row_comp=mysqli_fetch_array($run_comp);
                                        $compnamename=$row_comp['company_title'];
                                    }

                                    $fg_prices="SELECT price FROM `sb_products_attribute` where branch_code='$user->branch_code' AND product_code='$user->product_code'";
                                    $run_prices=mysqli_query($con,$fg_prices);
                                    if(mysqli_num_rows($run_prices) !=0)
                                    {
                                       $row_prices=mysqli_fetch_array($run_prices);
								...
								<td style="">{{$user->product_code}} - {{$user->product_name}}<br>
                                 <b>Company Name : </b> {{$user->company_code}} - {{$compnamename}}</td>
                                 <td style="text-align:center">Rs : {{$price}}</td>	
                                 	
								<td style="width:200px">
                                <select class="form-control statuschange" name="pro_status" id="pro_status" data-id='{{$user->id}}'  data-usercode='{{$usercode}}' data-branchid='{{$branchid}}' data-product_code='{{$user->product_code}}' 
											style='display:inline-block;font-size:13px;'>
												{{$status=$user->status;}}
												@if($status==1)
												<option value="1" >Active for All Dsf </option>
												<option value="0" >In-Active for All Dsf </option>
												@elseif($status==0)
												<option value="0" >In-Active for All Dsf </option>
												<option value="1">Active for All Dsf </option>
                                ...
                                <center>
                                <a href="<?php echo PROJECTURL ?>/lockparticulardsf/{{$branchid}}/{{$usercode}}/{{$user->id}}/{{$user->product_code}}/{{$user->product_name}}" target="_blank"
                                >
								<i class='fa fa-pen text-success' aria-hidden='true'></i></a>
                                </center>
				                </td>
  ...
  $(document).ready(function(){
       ...
       $('.statuschange').on('change',function(){
		...
		$.ajax({
            ...
            data: { id: id,status:status,usercode:usercode,branchid:branchid,product_code:product_code },
            dataType: 'json',
