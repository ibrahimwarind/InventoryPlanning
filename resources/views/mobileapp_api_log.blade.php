		...
		<div class="page-wrapper">
			<div class="page-content">
				...
				<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3" style="margin-top: -20px">
					<div class="breadcrumb-title pe-3">Mobile App Api Log</div>
					
					<div class="ms-auto">
						<div class="btn-group">
							
							<button type="button" class="btn btn-primary" style="padding:2px !important;font-size:15px;height:30px">Total Log Entry : {{number_format($apilogdatacount)}}</button>
				...
				<h6 class="mb-0 text-uppercase">
					<form method="get" action="<?php echo PROJECTURL ?>/mobileappapilog2">
						@csrf
					<div class="row">
						...
						<div class="col-sm-2  col-12">
							...
							<a href="<?php echo PROJECTURL ?>/mobileappapilog" class="btn btn-primary" style="padding:2px !important;font-size:15px;height:30px">Clear</a>
						</div>
					</div>
				   </form>
				</h6>
...
<!-- 				status , proname id code , brand , company  -->
						...
						<div class="">
							<table id="example" class="table table-striped table-bordered" >
								<thead>
									...
									<th style="text-align:center">Response Status</th>
									<th style="text-align:center">Product Count</th>
									<th style="text-align:center">Saf Count</th>	
									<th style="text-align:center">Log Date</th>	

									</tr>
								</thead>
								<tbody>
									@foreach($apilogdata as $user)
									<?php 
            ...
            $dsfname="";
            $fg_br="SELECT dsf_name FROM `sb_dsf_master` where dsf_code='$user->dsf_code'";
            $run_br=mysqli_query($con,$fg_br);
            if(mysqli_num_rows($run_br) !=0)
            {
            	$row_br=mysqli_fetch_array($run_br);
            	$dsfname=$row_br['dsf_name'];
            }
									?>
									<tr>
								<td style="text-align: center">{{$user->branch_code}} - {{$compname}}</td>
								<td style="text-align: center">{{$user->dsf_code}} - {{$dsfname}}</td>
								<td style="text-align: center">{{$user->response_status}}</td>
								<td style="text-align: center">{{$user->product_count}}</td>
								<td style="text-align: center">{{$user->saf_count}}</td>	
								<td style="text-align: center">{{date("d-M-Y h:i:s", strtotime($user->log_datetime))}}</td>
